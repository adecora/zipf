...
def test_integration():
    ...
    with open('test_data/random_words.txt') as reader:
        word_counts_dict = countwords.count_words(reader)
    counts_array = np.array(list(word_counts_dict.values()))
    actual_alpha = plotcounts.get_power_law_params(counts_array)
...
def test_alpha():
    """Test the calculation of alpha parameter.

    The test word couint satisfy the relationship,
        r = cf**(-1/alpha), where
        r is the rank
        f the word count, and
        c is a constant of proportionality.

    To generate test work counts for an expected alpha of
        1.0, a maximun word frequency of 600 is used
